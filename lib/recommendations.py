...
_GUIDANCE = {
    'M1': ('Memory and room to grow', 'Estimates memory use, how many database connections can fit, and room for more tables or shards (pieces of a table).', 'These estimates use the stated workload and memory settings. They are not a guarantee against running out of memory.', 'Check the assumptions below and test at peak load before increasing connections or adding data.'),
    ...
    'C3': ('Database connection capacity', 'Estimates direct connections and connections opened between servers for distributed work.', 'One application request can use several database connections.', 'Measure peak demand across all servers, including connections kept open for reuse.'),
